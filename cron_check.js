                    ...
                    if (result.code == 0 && result.data.code == 0) {
                        ...
                        if (status != account.packages[i].status
                            ...
                            if (result.code == 0 && result.data.code == 0) {
                                ...
                                if (get_one_order.pickup_time != 0) {
                                    delivery_time = moment.unix(get_one_order.delivery_time).format('YYYY-MM-DD HH:mm:ss');
                                }
                                let complete_time = null;
                                if (get_one_order.pickup_time != 0) {
                                    complete_time = moment.unix(get_one_order.complete_time).format('YYYY-MM-DD HH:mm:ss');
                                }
                                let cancel_time = null;
                                if (get_one_order.pickup_time != 0) {
                        ...
                        if (result.data.data.package_list.length > 0) {
                            ...
                            for (let i = 0; i < package_list.length; i++) {
                                ...
                                if (pack_time < max_pack_time && min_pack_time == -1) {
                                    ...
                                    if (min_pack_time == -1 || pack_time <= min_pack_time || min_pack_time == 0) {
                                        result = await shopeeApi.api_get_one_order(spc_cds, proxy, user_agent, cookie, order_id);
                                        last_request_success = moment();
                                        if (result.code == 0 && result.data.code == 0) {
                                            let get_one_order = result.data.data;
